.. programming for economists documentation master file, created by
   sphinx-quickstart on Mon Jun 20 12:48:40 2016.
   You can adapt this file completely to your liking, but it should at least
   contain the root `toctree` directive.
   sphinx-build -E ~/github/programming-for-economists/source ~/github/programming-for-economists



Welcome to Programming for Economists
=====================================

This is the webpage for the BSc course Programming for Economists at Tilburg
University. This course is taught in the first year for ECO and EBE.


**Note that this webpage is under construction for 2017/8**

Team
----

In 2017-2018 this course is taught by:

* Santiago Bohorquez Correa
* Jan Boone
* Clemens Fiedler
* Mark Klik
* Misja Mikkers
* Minke Remmerswaal

Make sure that you read the Rules of the Game if you want to attend
this course!

Datacamp
--------

We are very happy that we partner with `datacamp <https://www.datacamp.com/>`_
for this course to teach you both python and R.

`Datacamp <https://www.datacamp.com/about>`_ offers great on-line courses for
you to learn R and python and it allows us to track your progress...

.. _important:

Important
---------

Things to do if you want to follow this course:

* enroll in `datacamp <https://www.datacamp.com/>`_ **with your Tilburg University email address**
* sign up at `<github.com>`_
* create a repository called "assignments" as explained :ref:`here; <posting>`
  and save the address of this repository
* after you have done this, go to the webform:
* and fill in `this webform. <https://goo.gl/forms/AhgPhTw0L1TTGaNk2>`_
* note that you need to fill in the webform **before March 23, 2018**
* if you do not fill in the webform before the deadline, you *cannot* get a grade for this course
* we use the webform to plan the tutorials, keep an eye on BB before your first tutorial


Questions
---------

If you have a question about the course, use github as described :ref:`here. <questions>`


Contents
--------

.. toctree::
   :maxdepth: 1

   schedule
   rules
   install
   github
   python
   R
